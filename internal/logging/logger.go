...
func LoadLoggerConfig() (EnvLoggerConfig, error) {
	...
	if err := envconfig.Process(modules.GlobalEnvPrefix, &config); err != nil {
		return EnvLoggerConfig{}, errors.Wrap(err, "cannot get search-data-extractor logger config")
	}

	if config.LogLevel < -1 || config.LogLevel > 2 {
		return EnvLoggerConfig{}, errors.Wrap(errors.New("invalid log level"), "cannot get search-data-extractor logger level")
	}
